...
def infer_category(title: str) -> str:
    t = title.lower()
    if any(k in t for k in ["visage","peau","skincare","brosse","serum","crème","beaut"]): return "beauté"
    if any(k in t for k in ["lampe","led","veilleuse","projecteur","déco","deco","maison"]): return "maison"
    if any(k in t for k in ["sport","fitness","muscu","running","gourde","shaker"]): return "fitness"
    if any(k in t for k in ["cuisine","air fryer","poêle","mixeur","couteau"]): return "cuisine"
    if any(k in t for k in ["bébé","bebe","enfant","maman"]): return "bébé"
    if any(k in t for k in ["chien","chat","animaux","litière","laisse"]): return "animaux"
    if any(k in t for k in ["voiture","auto","moto"]): return "auto"
    return "autre"
...
def main():
    ...
    try:
        # 1) Collect (Trends actuel + fallback interne)
        ...
        # 3) Filter "sellable product"
        sellable: List[Dict[str, Any]] = []
        for c in merged:
            title = c["title"]
            if is_sellable_product(title, geo=GEO):
                sellable.append(c)
        stats["candidates_sellable"] = len(sellable)

        # (si vraiment trop peu, on garde quand même un minimum)
        if len(sellable) < 10:
            stats["note"] = "low_sellable_count"

        # 4) Enrich Pinterest + TikTok (non bloquant)
        ...
        for c in sellable:
            ...
            c["signals"]["tiktok"] = tk
            if (tk.get("hits", 0) > 0) or (tk.get("views_estimate", 0) > 0):
        ...
        # 8) Analyse IA longue + rows Supabase
        rows = []
        for w in winners:
            title = w["title"]
